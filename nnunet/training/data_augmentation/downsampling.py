...
def downsample_seg_for_ds_transform2(seg, ds_scales=((1, 1, 1), (0.5, 0.5, 0.5), (0.25, 0.25, 0.25)), order=0, axes=None):
    ...
    # seg = np.split(seg, 2, axis=1)   #gaidong
    output = []
    output.append(seg)
    # for s in ds_scales:
    #     if all([i == 1 for i in s]):
    ...
    #         output.append(seg[0])
    #     else:
    #         new_shape = np.array(seg[0].shape).astype(float)  ################gaidong seg -> seg[0]
    #         for i, a in enumerate(axes):
    #             new_shape[a] *= s[i]
    #         new_shape = np.round(new_shape).astype(int)
    #         out_seg = np.zeros(new_shape, dtype=seg[0].dtype)    ################gaidong seg -> seg[0]
    #         for b in range(seg[0].shape[0]):            ################gaidong seg -> seg[0]
    #             for c in range(seg[0].shape[1]):        ################gaidong seg -> seg[0]
    #                 out_seg[b, c] = resize_segmentation(seg[0][b, c], new_shape[2:], order)
