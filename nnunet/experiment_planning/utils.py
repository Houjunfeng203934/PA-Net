    ...
    for tr in training_files:
        ...
        for mod in range(num_modalities):
            cur_pat.append(join(base_folder_splitted, "imagesTr", tr['image'].split("/")[-1][:-7] +
                                "_%04.0d.nii.gz" % mod))
        cur_pat.append(join(base_folder_splitted, "labelsTr", tr['label'].split("/")[-1]))############change
        # cur_pat.append(join(base_folder_splitted, "labelsTr", tr['label'][0].split("/")[-1]))
        # cur_pat.append(join(base_folder_splitted, "labelsTr", tr['label'][1].split("/")[-1]))
