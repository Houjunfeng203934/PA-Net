...
sys.path.append("/home/hjf/daima/MAML-main")
...
if __name__ == "__main__":
    train_dir_V = "/home/hjf/dataset1/Mutul_data/888V"
    train_dir_A = "/home/hjf/dataset1/Mutul_data/888A"
    label_dir_V = "/home/hjf/dataset1/Mutul_data/888V_merge"
    label_dir_A = "/home/hjf/dataset1/Mutul_data/888A_merge"
    test_dir2 = "/home/hjf/dataset1/Mutul_data/testa"
    test_dir1 = "/home/hjf/dataset1/Mutul_data/testv"

    output_folder = "/home/hjf/daima/MAML-main/nnUNet_raw_data_base/nnUNet_raw_data/Task999_cao"
    ...
    def load_save_train(args):
        data_file_V, data_file_A, seg_file_V, seg_file_A = args
    ...
    nii_files_tr_data_V = subfiles(train_dir_V, True, "patient", "nii", True)  # 获得train_dir中包含volume,且以nii结尾的数据

    nii_files_tr_data_A = subfiles(train_dir_A, True, "patient", "nii", True)  # 获得train_dir中包含volume,且以nii结尾的数据

    nii_files_tr_seg_V = subfiles(label_dir_V, True, "patient", "nii", True)  # 获得train_dir中包含segmen，且以nii结尾的数据

    nii_files_tr_seg_A = subfiles(label_dir_A, True, "patient", "nii", True)  # 获得train_dir中包含segmen，且以nii结尾的数据

    nii_files_ts_A = subfiles(test_dir2, True, "patient", "nii", True)  # 获得test_dir中包含volume，且以nii结尾的数据

    nii_files_ts_V = subfiles(test_dir1, True, "patient", "nii", True)  # 获得test_dir中包含volume，且以nii结尾的数据
    p = Pool(default_num_threads)
    train_ids = p.map(load_save_train, zip(nii_files_tr_data_V, nii_files_tr_data_A, nii_files_tr_seg_V, nii_files_tr_seg_A))
    ...
    json_dict['training'] = [{'image': "./imagesTr/%s.nii.gz" % i, "label": ["./labelsTr/%s.nii.gz" % i,"./labelsTr/%s.nii.gz" % i.replace("V","A")]} for i in
                             train_ids]
