...
class attention(SegmentationNetwork):
    DEFAULT_BATCH_SIZE_3D = 2
    DEFAULT_PATCH_SIZE_3D = (64, 192, 160)
    SPACING_FACTOR_BETWEEN_STAGES = 2
    BASE_NUM_FEATURES_3D = 30
    ...
    def __init__(self, modality_num, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for i in range(modality_num):
            self.modality_specific_models.append(
                Generic_UNet(1, base_num_features, num_classes, num_pool, num_conv_per_stage,
        ...
        for i in range(modality_num):
            self.modality_aware_modules.append(
                nn.Sequential(conv_op(base_num_features, base_num_features, **self.conv_kwargs),
    ...
    def forward(self, x):
        ...
        for i in range(self.modality_num):
            ms_outputs = self.modality_specific_models[i](x[i])  # 第一个模态送入unet网络
        ...
        final_outputs = final_outputs[:6]
        # attention_maps = []
        # for i in range(self.modality_num):
        #     attention_maps.append(
        #         self.modality_aware_modules[i](modality_features[i]))
        #
        ...
        # final_outputs = [output, ] + final_outputs[:6]
